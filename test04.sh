...
(
    # empty command
    seq 1 10 | 2041 speed ';4q'
    echo $?

    # using either a ';' or a '\n' to seperate commands
    seq 1 10 | 2041 speed '2p;3q'
    echo $?
    ...
    # lots of commands
    seq 1 30 | 2041 speed '1p; 2d; /3/s/3/q/g; /4/p; 23q;'
    echo $?

    # Overlapping commands
    seq 1 30 | 2041 speed '/1/p;11d;/1./s/../-/g'
    echo $?

    # Test order matters
    # once deleted don't activate other commands for that affected line
    seq 1 30 | 2041 speed '/1/d; /1/p;'
    echo $?
    seq 1 30 | 2041 speed '/2/d; /.2/s/../-/g'
    echo $?
    seq 1 30 | 2041 speed '/2/p; /1/d; /1/p;'
    echo $?
    seq 1 30 | 2041 speed '/1/p; /5/p; /2/d; /.2/s/../-/g'
    echo $?
    #still apply the commands on the affected line if it comes before the delete
    seq 1 30 | 2041 speed '/1/p; /2/p; /2/d; /.2/s/../-/g'
    echo $?

    # once quitted don't activate other commands
    seq 1 30 | 2041 speed '/1/q; /1/p;'
    echo $?
    seq 1 30 | 2041 speed '/2/q; /.2/s/../-/g'
    echo $?
    seq 1 30 | 2041 speed '/2/p; /1/q; /1/p;'
    echo $?
    seq 1 30 | 2041 speed '/1/p; /5/p; /2/q; /.2/s/../-/g'
    echo $?
    #still apply the commands on the affected line if it comes before the delete
    seq 1 30 | 2041 speed '/1/p; /2/p; /2/q; /.2/s/../-/g'
    echo $?

    # delete and quit has the same order of priority 
    seq 1 15 | 2041 speed '/1/p; 2d; 2q;' #deleting the quit line will result in not quitting
    echo $?
    seq 1 15 | 2041 speed '/1/p; 2q; 2d;' #qutting on the delete line will result in not deleteing
    echo $?

    # print and substitute don't restrict other commands (but still order matters)
    seq 1 15 | 2041 speed '/1/p; /1/s/.*/hi/;'
    echo $?
    seq 1 15 | 2041 speed '/1/s/.*/hi/;/1/p;'
    echo $?
) >>"sol.txt" 2>>"sol.txt"
cd ..
NC='\033[0m' # No Color
